...
def store_department_list(fetch_child=True, parent_id=1):
    ...
    #print is_success, result #debug only
    if is_success == True:
...
def store_user_detail(department_id, offset=None, size=None, order=None):
    db, cursor = connect_db('localhost', 'root', 'yoyoball', 'dingtalk')
    #print dingapi_timer.access_token
...
if __name__=='__main__':
    ...
    for i in range(5):
        ...
        if i == 4 and status == False:
            print 'get_sub_dept_id_list error' #debug only 
            exit(-1)
    ...
    if ding_dept_num == None:
        print 'ding_dept_num is none'
        exit(-2)
    group_factor = 100.0 #设定分组单位，需要小数位为零
    ...
    for i in range(group_number):        
        ...
        for j in range(len(dept_group)):
            user_pool.apply_async(store_user_detail, args=(dept_group[j][0],))
    user_pool.close()
    user_pool.join()
